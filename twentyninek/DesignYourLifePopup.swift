...
class DesignYourLifePopup: UIView, UIScrollViewDelegate {
    ...
    let infoLabel: UITextView = {
        ...
        label.text = "Find out where to look for flow by logging your activities for one week."
    ...
    let pageOneImage: UIImageView = {
        let img = UIImageView()
        img.image = UIImage(named: "FindYourFlowPageOne")?.withRenderingMode(.alwaysOriginal)
    ...
    let pageTwoImage: UIImageView = {
        let img = UIImageView()
        img.image = UIImage(named: "FindYourFlowPageTwo")?.withRenderingMode(.alwaysOriginal)
        img.contentMode = .scaleAspectFit
        return img
    }()
